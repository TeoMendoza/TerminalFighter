...
@export var block_on_cooldown: bool = false
...
const character: String = "Ninja"
enum States {IDLE, MOVING, JUMPING, ABILITY1, FALLING, ABILITY2, KNOCKBACK}
...
func _physics_process(delta):
	if alive:
		if get_multiplayer_authority() == multiplayer.get_unique_id() and not animation_locked:
...
func handle_state_transitions():
	...
	elif (Input.is_action_just_pressed("dash") and get_multiplayer_authority() == multiplayer.get_unique_id()) or dashing:
	...
	elif (Input.is_action_just_pressed("block") and get_multiplayer_authority() == multiplayer.get_unique_id()) or blocking:
		if not is_using_ability:
			state = States.ABILITY2
			
	# Velocity check is for state preservation, same for falling
	elif (Input.is_action_just_pressed("jump") and get_multiplayer_authority() == multiplayer.get_unique_id()) or velocity.y < 0: 
...
func perform_state_actions(delta):
	...
	match state:
		...
		States.JUMPING:
			if (Input.is_action_just_pressed("jump") and get_multiplayer_authority() == multiplayer.get_unique_id()) and jumps > 0:
					use_passive_ability()
					jump()
		...
		States.FALLING:
			# Temp animation
			if current_animation != "Jump_Transition" and current_animation != "Fall" and not animation_locked:
				anim_sprite.play("JumpTransition")
				current_animation = "Jump_Transition"
			elif current_animation != "Fall" and not animation_locked and not anim_sprite.is_playing(): # Only plays fall animation once
				anim_sprite.play("Fall")
				current_animation = "Fall"
			velocity.y += gravity * delta * 1.5 # This is gravity * 2.5, since regular gravity is applied to each state, so its just adding 1.5 more
			velocity.x = speed * direction
			
		States.ABILITY2:
			use_ability_2(delta)
			# Allows horizontal movement during block
			velocity.x = speed * direction
...
func use_ability_1(delta):
	# If player is in a dash, process dash is what handles the timer, movement, and collision checks during the dash
	if dashing:
		ability1.process_dash(self, delta)
	elif not dash_on_cooldown:
		ability1.execute(self)
		
func use_ability_2(delta):
	# If player is in a block, process block is what handles the timer checks during the block
	if blocking:
		ability2.process_block(delta)
	elif not block_on_cooldown:
...
func _on_animated_sprite_2d_animation_finished():
	if anim_sprite.animation in ["Hurt", "Death", "Attack", "Dash", "Block"]: # Each of these animations means animation is locked, so when they end, it resets the bool
		animation_locked = false
